'''
Current clamp demo using Allen Brain's model 472455509.
...
from neuron472455509 import Neuron472455509
...
def demo(iapp):
    ...
    h.celsius = 34.0
    cell = Neuron472455509(name='neuron')
    ...
    vbox.intercept(0)
    vbox.map('Allen Institute model 472455509; v includes junction potential', 10, 10, 600, 400)

    # procedure for doing and plotting each simulation
    def do_current_clamp_experiment(amp, color):   
        ic.amp = amp / 1000.
        
        h.finitialize(-86.5537134806)
        h.fcurrent()
